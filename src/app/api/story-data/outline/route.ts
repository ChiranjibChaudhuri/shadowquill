...
export async function PUT(req: Request) {
    try {
        ...
        if (!storyId) return NextResponse.json({ error: 'Missing storyId' }, { status: 400 });
        if (typeof outline === 'undefined' && typeof numChapters === 'undefined') {
             return NextResponse.json({ error: 'Missing data to update (outline or numChapters)' }, { status: 400 });
        }

        // TODO: Auth check

        const dataToUpdate: { outlineText?: string; numChapters?: number } = {};
        if (typeof outline !== 'undefined') dataToUpdate.outlineText = outline;
        if (typeof numChapters !== 'undefined') dataToUpdate.numChapters = numChapters;
